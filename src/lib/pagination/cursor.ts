...
export function decodeCursor(cursor: string): CursorPayload | null {
  try {
    ...
    // Validate structure
    if (typeof payload.sortValue !== "string" || typeof payload.id !== "string" || !payload.sortValue || !payload.id) {
      return null;
    }
 ...
 * ```
 */
export function extractCursor(lastItem: { id: string; [key: string]: unknown }, sortField: string): string {
