...
export function DashboardView({ userEmail }: DashboardViewProps) {
  const { decks, loadingInitial, error, page, loadingMore, refresh, loadMore, query, setQuery, setDecks } =
    useDecksList();
  ...
  const updateActionState = React.useCallback((deckId: DeckId, next: DeckActionStateById[DeckId]) => {
    setActionStateById((prev) => ({ ...prev, [deckId]: next }));
  }, []);

  const clearActionState = React.useCallback((deckId: DeckId) => {
    setActionStateById((prev) => {
      const { [deckId]: removed, ...rest } = prev;
      void removed;
      return rest;
    });
  }, []);
  ...
  const handleRenameSubmit = React.useCallback(
    async (deckId: DeckId, patch: UpdateDeckCommand) => {
      ...
      setDecks((prev) => prev.map((deck) => (deck.id === deckId ? optimisticDeck : deck)));
      ...
      try {
        ...
        setDecks((prev) => prev.map((deck) => (deck.id === deckId ? toDeckListItemVm(updated) : deck)));
        ...
        setDecks((prev) => prev.map((deck) => (deck.id === deckId ? original : deck)));
        updateActionState(deckId, {
          isRenaming: false,
          isDeleting: false,
  ...
  }, [clearActionState, currentDeck.deckId, decks, deleteDeckId, setCurrentDeck, setDecks, updateActionState]);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-6 py-8">
        ...
        <CreateDeckDialog open={createDialogOpen} onOpenChange={setCreateDialogOpen} onCreated={handleCreatedDeck} />
