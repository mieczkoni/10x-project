...
import type { CardDto, CardId, DeckDto, DeckId, UpdateDeckCommand } from "../../../types"
...
export function DeckDetailView({ deckId, userEmail }: DeckDetailViewProps) {
  ...
  const handleCardSaved = React.useCallback(
    async (_card: CardDto) => {
      await cardsState.refresh()
    },
    [cardsState]
  ...
  const updateCardAction = React.useCallback(
    (cardId: CardId, patch: Partial<CardActionStateById[CardId]>) => {
      setCardActions((prev) => ({
        ...prev,
        [cardId]: {
          isUpdating: false,
