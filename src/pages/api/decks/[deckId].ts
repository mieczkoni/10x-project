...
export const GET: APIRoute = async ({ locals, params }) => {
  ...
  try {
    ...
    if (error instanceof ZodError) {
      return ApiErrors.invalidInput("Invalid deck ID format", { issues: JSON.parse(JSON.stringify(error.errors)) });
    }

    // Handle unexpected errors
    logger.error("[GET /api/decks/:deckId] Unexpected error", error);
...
export const PATCH: APIRoute = async ({ locals, params, request }) => {
  ...
  try {
    ...
    // Handle unexpected errors
    logger.error("[PATCH /api/decks/:deckId] Unexpected error", error);
    return ApiErrors.serverError();
  }
};
...
export const DELETE: APIRoute = async ({ locals, params }) => {
  ...
  try {
    ...
    // Handle unexpected errors
    logger.error("[DELETE /api/decks/:deckId] Unexpected error", error);
    return ApiErrors.serverError();
  }
};
